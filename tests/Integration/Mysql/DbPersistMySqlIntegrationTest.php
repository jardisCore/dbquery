...
/**
 * MySQL Integration Tests for DbPersist
 *
 * Tests actual INSERT/UPDATE/DELETE query execution against MySQL database
 */
class DbPersistMySqlIntegrationTest extends TestCase
{
    ...
    public function testInsertMultipleRows(): void
    {
        ...
        $stmt1 = $this->connection->prepare($prepared1->sql());
        $stmt1->execute($prepared1->bindings());

        // Insert second user
        $prepared2 = $this->persist->insert(
            'users',
        ...
        $stmt2 = $this->connection->prepare($prepared2->sql());
        $stmt2->execute($prepared2->bindings());
    ...
    public function testDeleteByPrimaryKey(): void
    {
        ...
        // Delete first user
        $prepared = $this->persist->delete('users', 'id', $userIdToDelete, 'mysql');
    ...
    public function testDeleteNonExistentRecord(): void
    {
        ...
        $prepared = $this->persist->delete('users', 'id', 99999, 'mysql');
    ...
    public function testDeleteAllRecords(): void
    {
        ...
        foreach ($rows as $row) {
            $prepared = $this->persist->delete('users', 'id', $row['id'], 'mysql');
            $stmt = $this->connection->prepare($prepared->sql());
            $stmt->execute($prepared->bindings());
        }
    ...
    public function testCompleteCrudCycle(): void
    {
        ...
        // 3. DELETE
        $deletePrepared = $this->persist->delete('users', 'id', $userId, 'mysql');
